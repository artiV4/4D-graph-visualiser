	...
	bool nextIteration(){
		...
		for (y = miny; y<maxy; y+=stepsize){
			for (z=minz; z<maxz; z+= stepsize){
				...
				if (verts[0] >= 0) {
					tempVert.emplace_back(x+stepsize*vertTable[verts[0]][0]);
					tempVert.emplace_back(y+stepsize*vertTable[verts[0]][1]);
					tempVert.emplace_back(z+stepsize*vertTable[verts[0]][2]);
					tempVert.emplace_back(x+stepsize*vertTable[verts[1]][0]);
					tempVert.emplace_back(y+stepsize*vertTable[verts[1]][1]);
					tempVert.emplace_back(z+stepsize*vertTable[verts[1]][2]);
					tempVert.emplace_back(x+stepsize*vertTable[verts[2]][0]);
					tempVert.emplace_back(y+stepsize*vertTable[verts[2]][1]);
					tempVert.emplace_back(z+stepsize*vertTable[verts[2]][2]);
				}
				if (verts[3] >= 0) {
					tempVert.emplace_back(x+stepsize*vertTable[verts[3]][0]);
					tempVert.emplace_back(y+stepsize*vertTable[verts[3]][1]);
					tempVert.emplace_back(z+stepsize*vertTable[verts[3]][2]);
					tempVert.emplace_back(x+stepsize*vertTable[verts[4]][0]);
					tempVert.emplace_back(y+stepsize*vertTable[verts[4]][1]);
					tempVert.emplace_back(z+stepsize*vertTable[verts[4]][2]);
					tempVert.emplace_back(x+stepsize*vertTable[verts[5]][0]);
					tempVert.emplace_back(y+stepsize*vertTable[verts[5]][1]);
					tempVert.emplace_back(z+stepsize*vertTable[verts[5]][2]);
				}
				if (verts[6] >= 0) {
					tempVert.emplace_back(x+stepsize*vertTable[verts[6]][0]);
					tempVert.emplace_back(y+stepsize*vertTable[verts[6]][1]);
					tempVert.emplace_back(z+stepsize*vertTable[verts[6]][2]);
					tempVert.emplace_back(x+stepsize*vertTable[verts[7]][0]);
					tempVert.emplace_back(y+stepsize*vertTable[verts[7]][1]);
					tempVert.emplace_back(z+stepsize*vertTable[verts[7]][2]);
					tempVert.emplace_back(x+stepsize*vertTable[verts[8]][0]);
					tempVert.emplace_back(y+stepsize*vertTable[verts[8]][1]);
					tempVert.emplace_back(z+stepsize*vertTable[verts[8]][2]);
				}
				if (verts[9] >= 0) {
					tempVert.emplace_back(x+stepsize*vertTable[verts[9]][0]);
					tempVert.emplace_back(y+stepsize*vertTable[verts[9]][1]);
					tempVert.emplace_back(z+stepsize*vertTable[verts[9]][2]);
					tempVert.emplace_back(x+stepsize*vertTable[verts[10]][0]);
					tempVert.emplace_back(y+stepsize*vertTable[verts[10]][1]);
					tempVert.emplace_back(z+stepsize*vertTable[verts[10]][2]);
					tempVert.emplace_back(x+stepsize*vertTable[verts[11]][0]);
					tempVert.emplace_back(y+stepsize*vertTable[verts[11]][1]);
					tempVert.emplace_back(z+stepsize*vertTable[verts[11]][2]);
				}
				if (verts[12] >= 0) {
					tempVert.emplace_back(x+stepsize*vertTable[verts[12]][0]);
					tempVert.emplace_back(y+stepsize*vertTable[verts[12]][1]);
					tempVert.emplace_back(z+stepsize*vertTable[verts[12]][2]);
					tempVert.emplace_back(x+stepsize*vertTable[verts[13]][0]);
					tempVert.emplace_back(y+stepsize*vertTable[verts[13]][1]);
					tempVert.emplace_back(z+stepsize*vertTable[verts[13]][2]);
					tempVert.emplace_back(x+stepsize*vertTable[verts[14]][0]);
					tempVert.emplace_back(y+stepsize*vertTable[verts[14]][1]);
					tempVert.emplace_back(z+stepsize*vertTable[verts[14]][2]);
				}
			}
		}
...
int main( int argc, char* argv[])
{
	...
	if (argc > 7) {
		...
		if (func==1) selectedF=f1;
		else if (func==2) selectedF=f2;
		else if (func==3) selectedF=f3;
		if (func==4) selectedF=f4;
	...
	window = glfwCreateWindow( screenW, screenH, "Marching   :   251239450", NULL, NULL);
	if( window == NULL ){
	...
	// Render then draw
	if (!liveRender){
		marchingVerts = marching_cubes(selectedF, isoval, xmin, xmax, ymin, ymax, zmin, zmax, stepsize);
	...
	do{
		if(liveRender && rendering){
			// render for next x value
			
			rendering = contRend.nextIteration();
			
			// refill the buffers and attribute pointers each time
			glBindVertexArray(vaoID);
